...
while str(hora)<'21:00':
   ...
   cursor.execute("SELECT * FROM view_ventas_caribu_py where PRIMER_Y_SEGUNDO_NOMBRE not like '%Sergio%'")
   ...
   for row in registros:
      ...
      #    if errorRfc==1:
      #       cursor.execute(f"update ventas_caribu set Observaciones = 'El RFC no corresponde', Estatus = 'No Activada' where IdActivacion = '{IdActivacion}'") 
      #       conex.commit()
      #       continue
      # except:
      #    pass
      ...
      print(f"EQUIS{pos[0]}")
      print(f"y{pos[1]}")
      pyautogui.dragTo(383, 295, 2, button="left")
      pos2=pos[0]-3
      time.sleep(2)
      pyautogui.dragRel(380, None, 2, button="left")
      time.sleep(2)
      pyautogui.hotkey('ctrl','c')
      time.sleep(1)
      numFolio=clipboard.paste()
      cursor.execute(f"update ventas_caribu set Observaciones = 'Sin Observaciones', Estatus = 'Activada',folio={numFolio}, Capturista='SYSPy' where IdActivacion = '{IdActivacion}'") 
      conex.commit()
      os.system("pause")
      time.sleep(999)
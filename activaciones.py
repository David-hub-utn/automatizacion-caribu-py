...
while str(hora)<'21:00':
   ...
   print(f"EQUIS{pos[0]}")
   print(f"y{pos[1]}")
   pyautogui.dragTo(382, 292, 0.5, button="left")
   pos2=pos[0]-3
   time.sleep(2)
   pyautogui.dragRel(379, None, 0.5, button="left")
   time.sleep(2)
   pyautogui.hotkey('ctrl','c')
   time.sleep(1)
   numFolio=clipboard.paste()
   #if len(numFolio)>11:
   #   cursor.execute(f"update ventas_caribu set Fecha_captura = '{PlanesFiltro}', FechaCaptura = getdate(), Observaciones = '{numFolio}', Estatus = 'No Activada' where IdActivacion = '{IdActivacion}'") 
   #   conex.commit()
   #else:
